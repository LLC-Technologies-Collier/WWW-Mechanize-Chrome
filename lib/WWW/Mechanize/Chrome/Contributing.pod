...
While the browser correctly renders XHTML using an XML parser, the CDP method
C<DOM.performSearch> (used by C<xpath()>, C<selector()>, and C<links()>)
re-parses the document bytes using a "tagsoup" HTML parser. This causes issues
with CDATA sections and script tags that contain literal C<< </script> >>
strings, leading to incorrect node identification.

=head3 Proposed Resolution

To resolve this within this module, we should implement a JavaScript-based
